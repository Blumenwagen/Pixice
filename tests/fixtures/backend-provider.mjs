...
export class BackendFixtureProvider extends EventEmitter {
  constructor(id = 'codex') { super(); this.id = id; this.connected = false; this.threads = new Map(); this.starts = 0; }
  ...
  event(payload) { this.emit('event', { type: 'RuntimeEvent', payload }); }
  async request(method, params = {}) {
    ...
    if (method === 'thread/list') return { data: [...this.threads.values()].filter(t => !params.cwd || t.cwd === params.cwd), nextCursor: null };
