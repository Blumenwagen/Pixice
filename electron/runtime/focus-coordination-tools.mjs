...
export const focusCoordinationShapes = {
  ...
  read_work: { workId: id },
  dispatch_work: { title: z.string().trim().min(1).max(160), prompt, model: id.optional(), effort: z.string().trim().regex(/^[a-z][a-z0-9_-]*$/i).max(32).optional(), access: z.enum(["read", "write"]).default("write"), resources: z.array(z.string().min(1).max(500)).max(30).optional(), dependsOn: z.array(id).max(20).optional() },
  follow_up: { workId: id, prompt },
...
export const focusCoordinationTools = [
  ...
  tool("dispatch_work", "Queue a bounded outcome and return immediately. Workers run asynchronously within the project's concurrency/access policy. Declare read vs write access and overlapping file/directory resources; unknown write scope serializes. Use existing work for follow-ups. Completion returns for coordinator review, not automatic success.", { title: string(), prompt: string(100000), model: string(), effort: string(32), access: { type: "string", enum: ["read", "write"] }, resources: { type: "array", items: string(500), maxItems: 30 }, dependsOn: ids }, ["title", "prompt"]),
  tool("follow_up", "Redirect or continue the existing outcome and worker. Active work receives steering; finished work is queued for continuation. Does not create a duplicate outcome.", { workId: string(), prompt: string(100000) }, ["workId", "prompt"]),
